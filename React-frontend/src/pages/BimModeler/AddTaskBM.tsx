...
export default function AddTaskBM() {
    ...
    const confirmRemoveAttachment = () => {
        ...
        if (pendingAttachmentDelete.type === "local") {
            setAttachmentFiles((prev) => prev.filter((_, i) => i !== (pendingAttachmentDelete as { index: number }).index));
    ...
    const getAssignToOptions = () => {
        ...
        const filteredEmployees = employees.filter(e => {
            ...
            const empName = e.full_name.replace(/\u00a0/g, " ").trim().toLowerCase();
                ...
                >
                    ...
                    >
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                            <h3 className="text-lg font-semibold text-[#353535]">Remove Attachment</h3>
                        </div>
                        <div className="px-6 py-8 text-center text-[#1A1A1A]">
                            <p className="mb-1">Are you sure you want to remove this attachment?</p>
                            <p className="font-semibold truncate max-w-full px-4">{pendingDeleteFileName}</p>
                        </div>
