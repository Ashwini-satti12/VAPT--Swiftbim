...
export default function TimesheetPM() {
  ...
  const selectedRangeOpt =
    showEntriesOptions.find((o) => o.value === effectiveShowEntryValue) ?? showEntriesOptions[0];
  const rangeStart = selectedRangeOpt.start;
  const rangeEnd = selectedRangeOpt.end === null ? filteredList.length : Math.min(selectedRangeOpt.end, filteredList.length);
    ...
    <div className="p-1 space-y-8 flex flex-col h-full bg-white">
      ...
      <div className="flex flex-col gap-4 flex-shrink-0 px-1 sm:px-0 -mt-1 sm:mt-0">
        ...
        <div className="flex items-center justify-between w-full">
          <h3 className="text-[20px] sm:text-[24px] font-medium text-[#000000] font-gantari whitespace-nowrap">Monthly Report</h3>
          <div className="relative group">
            <button
              onClick={handleDownload}
              disabled={filteredList.length === 0}
              className="flex items-center gap-2 px-4 sm:px-6 py-1.5 sm:py-2 bg-[#DD4342] text-white rounded-md font-gantari font-semibold cursor-pointer"
            >
              <svg
                width="20"
                height="20"
