...
export default function ConsultantdetailsBL() {
    ...
    if (!emp) return <div className="text-center py-12 text-slate-500">Consultant not found. <Link to="/bl/consultants" className="text-[#3d3399] hover:underline cursor-pointer">Back to Consultants</Link></div>;

    const formatDate = (d?: string) => (d ? new Date(d).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '-');

    return (
        <div className="space-y-4">
            <Link to="/bl/consultants" className="text-[#3d3399] hover:underline text-sm font-medium cursor-pointer">&larr; Back to Consultants</Link>
            <div className="bg-white rounded-xl border border-slate-200 p-6">
                <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                    ...
                    <Link to={`/bl/consultants/${emp.id}/edit`} className="px-4 py-2 rounded-lg border border-[#3d3399] text-[#3d3399] font-medium hover:bg-[#3d3399]/10 cursor-pointer">Edit</Link>
                </div>
