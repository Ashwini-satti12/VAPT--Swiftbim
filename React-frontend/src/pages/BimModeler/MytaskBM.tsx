    ...
    <div className="relative w-full">
      ...
      {isOpen && (
        <div
          ref={dropdownRef}
          role="listbox"
          className="absolute top-full left-0 z-20 mt-1 w-full rounded-md border border-slate-200 bg-white py-1"
        >
          {searchable && (
          ...
          <div className="max-h-60 overflow-y-auto py-1 custom-scrollbar">
            {filteredOptions.map((opt) => (
              <button
                ...
                className="block w-full px-3 py-2 text-left text-[14px] text-[#616161] hover:text-[#000000] hover:bg-[#F2F3F4] first:rounded-t-md last:rounded-b-md cursor-pointer"
              >
                {opt.label}
              </button>
            ))}
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`inline-flex items-center justify-between rounded-md bg-[#F2F3F4] px-4 py-2 text-[14px] text-[#353535] cursor-pointer ${narrow ? "min-w-[90px]" : "min-w-[140px]"}`}
        aria-expanded={isOpen}
        aria-haspopup="listbox"
        aria-label={label}
      >
      ...
      {isOpen && (
        <div
          ref={dropdownRef}
          role="listbox"
          className={`absolute top-full left-0 z-10 mt-1 w-full rounded-md border border-slate-200 bg-white  ${narrow ? "min-w-[110px]" : "min-w-[160px]"}`}
        >
          {searchable && (
            <div className="sticky top-0 bg-white p-2 rounded-md">
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                ...
                className="w-full rounded-md border border-[#A0A0A0] px-3 py-2 text-[14px] text-slate-800 placeholder-slate-400"
          ...
          >
            {filteredOptions.map((opt, idx) => (
              <button
                ...
                className={`block w-full px-4 py-2 text-left text-[14px] font-gantari transition-colors cursor-pointer ${selected === opt ? "bg-[#F2F3F4] text-[#353535]" : "text-[#616161] hover:text-[#000000] hover:bg-[#F2F3F4]"}`}
              >
                {opt}
              </button>
            ))}
    ...
    <li className="flex items-center gap-3 rounded-sm bg-[#F2F3F4] px-3 py-2 text-sm text-[#101827]">
      ...
      >
        {previewUrl ? (
          ...
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded border border-slate-200 bg-[#F2F3F4] text-[#616161] cursor-pointer">
            <svg
              className="h-6 w-6"
              fill="none"
              stroke="currentColor"
...
function TaskCard({
  ...
  return (
    <div
      ...
      className={`rounded-xl border border-slate-200 bg-white p-3 relative ${isCompleted ? "cursor-default" : "cursor-grab active:cursor-grabbing"}`}
    >
      <div className="flex items-center justify-between gap-2 mb-2">
        <h4 className="font-semibold text-slate-900 text-[16px] truncate">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div
              className={`absolute top-full mt-1 z-50 min-w-[160px] bg-white/20 backdrop-blur-md rounded-md border border-[#59595980] transition-all duration-200 ease-out ${isCompleted ? "right-full mr-1 origin-top-right" : "left-full ml-1 origin-top-left"}
...
export default function MytaskBM() {
  ...
  const handleMoveTask = (
    ...
    api
      ...
      .then(() => {
        setList((prev) =>
          prev.map((t) => (t && t.id === taskId ? { ...t, status: label } : t)),
        );
      });
  };
  ...
  useEffect(() => {
    ...
    if (selectedProj) {
      api
        .post("/api/projects/filters/modules", { projectId: selectedProj.id })
        .then(({ data }) => {
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4">
              <button
                ...
                className="p-1 rounded-md text-[#000000] bg-[#F2F3F4] transition-colors cursor-pointer"
                aria-label="Close"
              >
                <svg
                  className="w-5 h-5"
            ...
            <div className="flex justify-center gap-3 px-6 py-4 bg-slate-50/50">
              <button
                type="button"
                onClick={() => setDeleteTaskId(null)}
                className="rounded-md bg-[#F2F3F4] px-5 py-2 text-[16px] font-medium text-[#000000] cursor-pointer"
              >
                Discard
              </button>
              <button
                ...
                className="rounded-lg bg-[#FFD9D9] px-5 py-2 text-[16px] font-medium text-[#000000] cursor-pointer"
              >
                Yes, Delete
              </button>
            </div>
      ...
      {addTaskModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-[#FFFFFF] rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
              ...
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  if (isEditing) {
                    ...
                    const res = await api.post<{ task_id: number }>(
                      "/api/tasks",
                      payload,
                    );
                  ...
                  if (statusFilter) params.status = statusFilter;
                  if (ismy) params.condition = "1";
                  const updatedTasks = await api.get<{ tasks?: Task[] }>(
                    "/api/tasks",
                    { params },
                  );
            ...
            >
              {addError && (
                <div className="mb-4 flex items-start gap-3 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-[14px] text-red-700">
                  <div className="mt-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-100 text-[11px] font-bold">
                    !
                  </div>
                  <div className="flex-1">
                    <p className="text-[14px] leading-snug">{addError}</p>
                  </div>
                </div>
              )}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="text-[16px] font-medium text-[#353535] mb-1">
                ...
                <div>
                  ...
                  <div className="flex">
                    <input
                      ...
                      className={`flex-1 bg-[#F2F3F4] px-3 py-2 text-[14px] text-[#353535]${editingTaskId !== null ? "rounded-md" : "rounded-l-md"
                        }`}
                    />
                    {editingTaskId === null && (
                      <button
                        type="button"
                        className="rounded-l-none rounded-r-md bg-[#F2F3F4] px-4 py-2 text-[14px] font-medium text-[#8B8B8B] cursor-pointer"
                      >
                        Tasklist
                      </button>
                    )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    <div className="flex flex-1 min-w-0">
                      ...
                      <label
                        ...
                        className="rounded-r-md rounded-l-none bg-[#E5E7EB] px-4 py-2 text-[14px] font-medium text-[#000000] cursor-pointer inline-flex items-center"
                      >
                        Browse File
                      </label>
                    </div>
              ...
              <div className="flex justify-center gap-3 mt-6 pt-4 ">
                ...
                <button
                  ...
                  className="rounded-md bg-[#DBE9FE] px-5 py-2 text-[16px] font-medium text-[#353535] cursor-pointer"
                >
