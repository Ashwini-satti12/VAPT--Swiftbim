            ...
            <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-6 pb-4 overflow-visible lg:overflow-hidden">
                        ...
                        {[1, 2, 3, 4, 5, 6].map((_, idx) => (
                            ...
                            >
                                ...
                                <div className="relative w-20 h-20 flex items-center justify-center shrink-0">
                                    ...
                                    <span className="absolute text-[10px] font-bold text-black font-mono">hh:mm:ss</span>
                                </div>

                                {/* Task Info */}
                                <div className="flex-1 min-w-0 pr-2">
                                    <div className="mb-2">
                                        <h3 className="text-xl font-bold text-[#353535] truncate mb-0.5">Task Name</h3>
                ...
                <div className="lg:col-span-1 flex flex-col h-[600px] lg:h-full overflow-hidden">
                    <div className="bg-white rounded-2xl border border-[#AEACAC52] pt-4 pl-4 pb-4 pr-0 shadow-sm flex flex-col h-full overflow-hidden">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-6 lg:gap-12 mb-6 shrink-0 pt-2 px-2">
                            <div className="flex flex-col items-start min-w-[80px]">
                                <span className="text-[18px] lg:text-[20px] font-bold text-[#020202] font-gantari leading-tight lg:-ml-12">
                                    "December
                                </span>
                                <span className="text-[18px] lg:text-[20px] font-bold text-[#020202] font-gantari leading-tight lg:-ml-10">2026</span>
                            </div>
                            <div className="flex flex-col items-center">
                                <span className="text-[32px] lg:text-[40px] font-bold text-black leading-none tracking-tighter">13</span>
                            </div>
                            <div className="flex flex-col items-start min-w-[80px]">
                                <span className="text-[16px] lg:text-[20px] font-bold text-black font-gantari tracking-wide">SUNDAY</span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pl-2 pr-1 space-y-2">
                            ...
                            {isCalendarExpanded && (
                                <div className="mb-2 py-2 border-t border-slate-50 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <div className="grid grid-cols-7 gap-y-1 text-center text-[13px] font-bold text-slate-400 font-gantari mb-2">
                                        <div>S</div><div>M</div><div>T</div><div>W</div><div>T</div><div>F</div><div>S</div>
                                    </div>
                                    <div className="grid grid-cols-7 gap-y-2 text-center text-[15px] font-semibold font-gantari">
                                        {Array.from({ length: 31 }, (_, i) => i + 1).map(d => (
                                            <div key={d} className={`py-1 ${d === 13 ? 'text-[#E00100] font-bold' : 'text-[#020202]'}`}>{d}</div>
                                        ))}
                                    </div>
                                </div>
                            )}

                            {/* Toggle Arrow - Moved Below Calendar Grid */}
                            <div className="flex justify-center mb-4">
                                <button
                                    onClick={() => setIsCalendarExpanded(!isCalendarExpanded)}
                                    className="text-slate-400 hover:text-slate-600 transition-colors"
                                >
                                    <svg className={`w-7 h-6 transform transition-transform ${isCalendarExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
