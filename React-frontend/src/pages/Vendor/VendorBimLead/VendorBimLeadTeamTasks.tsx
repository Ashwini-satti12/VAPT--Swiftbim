...
export default function TeamtaskV() {
  ...
  const handleMoveTask = (taskId: number, newStatus: "todo" | "in_progress" | "completed") => {
    ...
    const newOverrides = { ...statusOverrides, [taskId]: label };
    setStatusOverrides(newOverrides);
    localStorage.setItem(STATUS_OVERRIDES_KEY, JSON.stringify(newOverrides));
    ...
    const nextLocal = localTasks.map(updateTaskState);
    setLocalTasks(nextLocal);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(nextLocal));

    const nextList = list.map(updateTaskState);
    setList(nextList);
  ...
  const handleApproveTask = (task: Task) => {
    const endpoint = `/api/vendors/vendor-tasks/${task.id}/status`;
    api.patch(endpoint, { status: "Approved" })
      .then(() => {
        toast.success("Task Approved");
        const update = (t: Task) => t.id === task.id ? { ...t, Approval: "Approved", progress: 100 } : t;
        setLocalTasks(prev => prev.map(update));
  ...
  useEffect(() => {
    ...
    return () => { delete (window as any).handleApproveTask; };
  }, [list, localTasks]);
  ...
  const openEditTask = (task: Task) => {
    navigate("/v/mytasks/add", { state: { task, from: "mytasks" } });
  };
  ...
  const confirmDeleteTask = () => {
    if (deleteTask !== null) {
      const endpoint = `/api/vendors/vendor-tasks/${deleteTask.id}`;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [tasksRes, resourcesRes, projRes] = await Promise.all([
        ...
        if (!cancelled) {
          toast.error("Failed to load tasks and resources.");
          setList([]);
          setLoading(false);
        }
      }
