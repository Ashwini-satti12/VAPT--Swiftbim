...
export default function Clients() {
  ...
  const [editForm, setEditForm] = useState({ fullName: '', email: '', phoneNumber: '', address: '', projectName: '', budget: '' });
  ...
  useEffect(() => {
    ...
    setEditId(id);
    api.get<Client & { address?: string; budget?: string }>(`/api/clients/${id}`).then(({ data }) => setEditForm({ fullName: data.fullName ?? '', email: data.email ?? '', phoneNumber: data.phoneNumber ?? '', address: data.address ?? '', projectName: data.projectName ?? '', budget: data.budget ?? '' })).catch(() => { });
  ...
  function handleEditSubmit(e: React.FormEvent) {
    ...
    api.patch(`/api/clients/${editId}`, { fullName: editForm.fullName, email: editForm.email, phoneNumber: editForm.phoneNumber || undefined, address: editForm.address || undefined, projectName: editForm.projectName || undefined, budget: editForm.budget || undefined }).then(() => { setList((prev) => prev.map((c) => (c.id === editId ? { ...c, fullName: editForm.fullName, email: editForm.email, projectName: editForm.projectName, address: editForm.address } : c))); setEditId(null); setSearchParams({}); }).catch(() => { }).finally(() => setEditSubmitting(false));
  }
