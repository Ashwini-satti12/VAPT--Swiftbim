  ...
  useEffect(() => {
    api.get<{ employees?: Employee[] }>('/api/employees').then(({ data }) => {
      ...
      withPics.slice(0, 5).forEach(emp => {
        const url = getGlobalProfileUrl(emp.id, emp.profile_picture);
                            ...
                            {emp.profile_picture && emp.profile_picture.trim() ? (
                              <img
                                ...
                                onError={(e) => {
                                  ...
                                  console.error(`❌ Image failed to load for ${emp.full_name}`, {
                                    src: target.src,
                                    profilePicture: emp.profile_picture,
                                    generatedUrl: getGlobalProfileUrl(emp.id, emp.profile_picture),
                                  });
                                  // Fallback to initials if image fails
                                  target.style.display = 'none';
                                  // Need to find the parent div and show the fallback initial
                                ...
                                onLoad={() => {
                                  console.log(`✅ Successfully loaded image for ${emp.full_name}:`, getGlobalProfileUrl(emp.id, emp.profile_picture));
                                }}
                              />
                            ) : (
              ...
              <div className="w-[100px] h-[100px] rounded-full overflow-hidden bg-[#F4F4F4] shrink-0">
                {selectedEmployee.profile_picture && selectedEmployee.profile_picture.trim() ? (
                  <img
                    ...
                    onError={(e) => {
                      const target = e.target as HTMLImageElement;
                      console.error(`❌ Modal image failed to load for ${selectedEmployee.full_name}`, {
                        src: target.src,
                        profilePicture: selectedEmployee.profile_picture,
                        generatedUrl: getGlobalProfileUrl(selectedEmployee.id, selectedEmployee.profile_picture),
                      });
                      target.style.display = 'none';
                      if (target.parentElement) {
                        const fallback = target.parentElement.querySelector('.fallback-initial');
                    ...
                    onLoad={() => {
                      console.log(`✅ Successfully loaded image for ${selectedEmployee.full_name}:`, getGlobalProfileUrl(selectedEmployee.id, selectedEmployee.profile_picture));
                    }}
                  />
                ) : (
                  <div className="w-full h-full bg-gray-200 flex items-center justify-center">
