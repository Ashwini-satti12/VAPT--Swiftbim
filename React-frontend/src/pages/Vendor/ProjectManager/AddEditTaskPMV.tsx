...
export default function AddEditTaskPMV() {
    ...
    if (editingTaskId) {
      api.patch(`/api/vendors/vendor-tasks/${editingTaskId}`, {
        ...
        if (attachmentFiles.length > 0) {
          ...
          attachmentFiles.forEach((f: File) => formData.append("image", f));
  ...
  const dynamicModuleOptions = useMemo(() => {
    const proj = projects.find((p: Project) => p.project_name === addTaskForm.projectName);
    return (proj?.modules || "").split(",").map((m: string) => m.trim()).filter(Boolean);
  ...
  const employeesForAssignDropdown = useMemo(() => {
    ...
    const members = (meta?.members || "").split(",").map((s: string) => s.trim()).filter(Boolean);
    if (members.length === 0) return employees;
    return employees.filter((e: Employee) => members.includes(String(e.id)) || members.includes(e.full_name));
  ...
  return (
    <div className="flex-1 min-h-0 bg-white overflow-hidden font-Gantari">
      ...
      <div className="max-w-[1174px] mx-auto h-full min-h-0 flex flex-col pt-6 px-6">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar pr-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            {addError && (
              <div className="mb-4 flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 shadow-sm font-Gantari">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
              <div className="md:col-span-2">
                ...
                <FormDropdown
                  ...
                  options={[{ value: "", label: "Select Project name" }, ...projects.map((p: Project) => ({ value: p.project_name, label: p.project_name }))]}
                  value={addTaskForm.projectName}
                  onChange={(v: string) => setAddTaskForm((f) => ({ ...f, projectName: v, module: "", assignTo: "" }))}
                  isOpen={openFormDropdown === "project"}
                  onToggle={() => setOpenFormDropdown((d: FormDropdownId) => (d === "project" ? null : "project"))}
              ...
              <div>
                ...
                <FormDropdown
                  label="Select Module"
                  options={[{ value: "", label: "Select Module" }, ...dynamicModuleOptions.map((m: string) => ({ value: m, label: m }))]}
                  value={addTaskForm.module}
                  onChange={(v: string) => setAddTaskForm((f) => ({ ...f, module: v }))}
                  isOpen={openFormDropdown === "module"}
                  onToggle={() => setOpenFormDropdown((d: FormDropdownId) => (d === "module" ? null : "module"))}
                  onClose={() => setOpenFormDropdown(null)}
                        ...
                        <div className="absolute top-full right-0 mt-1 z-50 w-80 rounded-lg border border-slate-200 bg-white shadow-lg max-h-96 overflow-y-auto">
                          {recentTasks.map((t: Task) => (
                            <button
                              ...
                              className="w-full px-4 py-2 text-left text-sm text-[#353535] hover:bg-slate-100 font-Gantari border-0 cursor-pointer"
                            >
                              {t.task_name}
                            </button>
                          ))}
              ...
              <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-6">
                {/* <div>
                  ...
                  <FormDropdown
                    ...
                    onChange={(v: string) => setAddTaskForm((f) => ({ ...f, type: v }))}
                    isOpen={openFormDropdown === "type"}
                    onToggle={() => setOpenFormDropdown((d: FormDropdownId) => (d === "type" ? null : "type"))}
                    onClose={() => setOpenFormDropdown(null)}
                ...
                <div>
                  ...
                  <FormDropdown
                    ...
                    options={[
                      ...
                      ...employeesForAssignDropdown.map((e: Employee) => ({ value: e.full_name, label: e.full_name }))
                    ]}
                    value={addTaskForm.assignTo}
                    onChange={(v: string) => setAddTaskForm((f) => ({ ...f, assignTo: v }))}
                    isOpen={openFormDropdown === "assignTo"}
                    onToggle={() => setOpenFormDropdown((d: FormDropdownId) => (d === "assignTo" ? null : "assignTo"))}
                    onClose={() => setOpenFormDropdown(null)}
              ...
              <div className="md:col-span-2 space-y-2">
                ...
                {totalAttachmentCount > 0 && (
                  <div className="flex flex-col gap-2">
                    {existingAttachmentNames.map((stored: string) => (
                      <div
                        key={`server-${stored}`}
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center pt-8">
              <button
                type="button"
                onClick={() => navigate(-1)}
                className="w-full sm:w-auto px-12 py-2 rounded-lg bg-[#F2F2F2] text-[#616161] font-semibold text-[16px] transition-all font-Gantari min-w-[160px] cursor-pointer border-0"
              >
                Discard
              </button>
              <button
                ...
                className="w-full sm:w-auto px-12 py-2 rounded-lg bg-[#DBE9FE] text-[#101827] font-semibold text-[16px] transition-all font-Gantari min-w-[160px] cursor-pointer disabled:opacity-50 border-0"
              >
      ...
      {pendingAttachmentDelete !== null && (
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 p-4 font-Gantari"
          onClick={() => !serverAttachmentDeleting && setPendingAttachmentDelete(null)}
          role="presentation"
        >
          <div
            className="w-full max-w-[520px] rounded-md bg-white p-8 shadow-xl"
          ...
          >
            <div className="relative">
              <button
                ...
                className="absolute right-0 top-0 shrink-0 rounded-lg p-1 bg-transparent cursor-pointer disabled:opacity-50 border-0 shadow-none hover:bg-slate-100"
                aria-label="Close"
              >
                <img src={closeButtonIcon} alt="" className="h-5 w-5" aria-hidden />
              </button>
              <p className="mx-auto max-w-full px-10 text-center text-[16px] leading-relaxed text-[#353535] font-Gantari">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
              <button
                ...
                disabled={serverAttachmentDeleting}
                onClick={() => setPendingAttachmentDelete(null)}
                className="min-w-[140px] rounded-md bg-[#F2F2F2] px-8 py-2 text-[14px] font-semibold text-[#353535] cursor-pointer transition-opacity hover:opacity-90 border-0 disabled:opacity-50 shadow-none font-Gantari"
              >
                Discard
              </button>
              <button
                ...
                className="min-w-[140px] rounded-md bg-[#FED9D9] px-8 py-2 text-[14px] font-semibold text-[#E00100] cursor-pointer transition-opacity hover:opacity-90 border-0 disabled:opacity-50 shadow-none font-Gantari"
              >
