...
function FormDropdown({
  ...
  const filteredOptions = searchable
    ? options.filter(
        (opt) =>
          opt.label.toLowerCase().includes(searchQuery.toLowerCase()) || opt.value === ""
      )
    : options;
    ...
    <li className="flex items-center gap-3 rounded-sm bg-[#F2F3F4] px-3 py-2 text-[14px] text-[#101827] font-Gantari">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <span className="text-[12px] text-[#8B8B8B]">{formatFileSize(file.size)}</span>
        </div>
      </button>
      <button
        type="button"
...
export default function AddTaskPM() {
  ...
  const formModuleTriggerRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    if (addTaskForm.projectName) {
      ...
      api
        .post<{ success: boolean; modules: { label: string }[] }>("/api/projects/filters/modules", {
          projectId: selectedProj.id,
        })
        .then((res) => {
          const list = Array.isArray(res.data.modules) ? res.data.modules : [];
          setModules(list.map((m: { label?: string }) => m?.label).filter(Boolean));
        })
        .catch(() => setModules([]));
    } else setModules([]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
            ...
            <div>
              ...
              <FormDropdown
                ...
                onChange={(v) => setAddTaskForm((f) => ({ ...f, taskName: v }))}
                isOpen={openFormDropdown === "taskName"}
                onToggle={() => setOpenFormDropdown((d) => (d === "taskName" ? null : "taskName"))}
                onClose={() => setOpenFormDropdown(null)}
            ...
            <div className="md:col-span-2">
              <label className="block text-[16px] font-semibold text-[#000000] mb-2 font-Gantari">Attachments</label>
              <input
                ref={fileInputRef}
