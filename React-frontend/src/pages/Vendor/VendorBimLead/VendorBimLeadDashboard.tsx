        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {kpiCards.map((card, i) => (
            <Link
              key={i}
              to={card.link}
              className="bg-[#F2F2F2] group hover:bg-[#DD4342] rounded-md border border-[#AEACAC52] px-4 py-3 flex items-center justify-between min-h-0 transition-colors cursor-pointer"
            >
              <h3 className="text-[18px] md:text-[18px] text-[#353535] group-hover:text-[#F2F2F2] font-medium font-gantari">
                {card.label}
              </h3>
              <p className="text-[20px] md:text-[20] text-[#353535] group-hover:text-[#F2F2F2] font-bold leading-none">
                {card.value}
              </p>
            </Link>
          ))}
        ...
        <div className="lg:col-span-2 flex flex-col bg-white rounded-2xl border border-[#AEACAC52] shadow-sm pt-4 pl-4 pb-4 pr-0 h-[500px] lg:h-full overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar min-h-0">
            {priorityTasks.length === 0 ? (
              <p className="text-[#717171] text-[16px] font-gantari px-4">
              ...
              <div className="px-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Link
                    ...
                    className="text-sm font-medium text-[#DD4342] hover:underline font-gantari transition-colors cursor-pointer"
                  >
                    View all
                  </Link>
                </div>
                    ...
                    ({ id, projectName, tasks: projectTasks }) => (
                      <div key={id} className="mb-6">
                        <p className="text-sm font-semibold text-[#353535] font-gantari mb-3 truncate pr-2">
                          <Link
                            to="/vendor-bim-lead/projects"
                            className="hover:text-[#DD4342] hover:underline transition-colors cursor-pointer"
                            title={projectName}
                          >
                            {projectName}
                          </Link>
                        ...
                        <div className="space-y-4">
                          {projectTasks.map((task) => {
                            ...
                            return (
                              <div
                                key={task.id}
                                className="flex items-center gap-5 p-5 bg-[#F8F8F8] rounded-xl border border-slate-200/80 shadow-sm relative transition-shadow hover:shadow-md"
                              >
                                ...
                                <div className="absolute bottom-4 right-4 flex -space-x-4">
                                  {(
                                    task.involved_persons?.slice(0, 3) || []
                                  ).map((person) => (
                                    <div
                                      key={person.id}
        ...
        <div className="lg:col-span-1 flex flex-col h-[600px] lg:h-full overflow-hidden">
          <div className="bg-white rounded-2xl border border-[#AEACAC52] pl-4 pb-4 pr-0 shadow-sm flex flex-col h-full min-h-0">
            ...
            <div className="flex flex-nowrap items-center justify-center gap-2 sm:gap-4 mb-3 px-2 shrink-0">
              ...
              <span className="text-[13px] font-semibold text-slate-700 font-gantari min-w-[80px] text-center">
                {currentMonthName}
              </span>
              <button
                type="button"
              ...
              <div className="space-y-4 pr-1">
                {!celebrationsRequested ? (
                  <p className="text-sm text-slate-400 font-gantari py-4 text-center">
                    Click the up arrow to load celebrations.
                  </p>
                  ...
                  celebrations.map((event, i) => (
                    ...
                    >
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span
                          className={`text-[10px] px-3 py-1.5 rounded-md font-bold uppercase tracking-widest leading-none font-gantari ${event.type === "birthday" ? "bg-[#FFF3E0] text-[#E65100]" : event.type === "work_anniversary" ? "bg-[#E7F6EA] text-[#2D8A39]" : "bg-[#E3F2FD] text-[#1565C0]"}`}
                        >
                      ...
                      <p className="text-sm text-slate-400 leading-relaxed font-gantari">
                        {event.type === "birthday"
                          ...
                          : event.type === "work_anniversary"
                            ? `Celebrating ${event.working_years} years!`
                            : `Project due: ${event.project_name}`}
                      </p>
                    </div>
                  ))
                )}
