...
export default function ClientTD() {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        {canAdd && (
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2 rounded-md bg-[#DD4342] text-white font-semibold transition shadow-md shadow-red-100 cursor-pointer"
          >
      ...
      <div
        className="flex-1 mt-6 overflow-y-auto"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredList.length === 0 ? (
            ...
            filteredList.map((c) => (
              <div
                key={c.id}
                className="bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 border border-slate-100 overflow-hidden flex flex-col group"
              >
                {/* Header Background */}
                <div className="relative h-44 w-full">
                  ...
                  <div className="absolute bottom-6 left-6 flex items-center gap-4 text-white">
                    ...
                    <div>
                      <h3 className="text-xl font-gantari leading-tight drop-shadow-md line-clamp-1">
                        {displayName(c)}
                      </h3>
                      <p className="text-sm text-[#F2F2F2] font-gantari drop-shadow-sm">
                        {displayLocation(c)}
                      </p>
                    </div>
                  </div>
                ...
                <div className="p-6 space-y-6 flex-1 flex flex-col">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <button
                      className="flex items-center justify-center gap-2 py-3 rounded-lg bg-[#DBE9FE] hover:bg-[#c9deff] transition-colors cursor-pointer"
                      title="Mail"
                    >
                      <img
                        src={mailIcon}
                      ...
                      />
                      <span className="text-sm font-bold text-[#12141D]">
                        Mail
                      </span>
                    </button>
                    <button
