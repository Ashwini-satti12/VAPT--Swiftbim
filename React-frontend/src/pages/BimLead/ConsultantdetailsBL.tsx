...
export default function ConsultantdetailsBL() {
    ...
    if (!emp) return <div className="text-center py-12 text-slate-500">Consultant not found. <Link to="/bl/consultants" className="text-[#3d3399] hover:underline">Back to Consultants</Link></div>;

    const formatDate = (d?: string) => (d ? new Date(d).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '-');

    return (
        <div className="space-y-4">
            <Link to="/bl/consultants" className="text-[#3d3399] hover:underline text-sm font-medium">&larr; Back to Consultants</Link>
