        ...
        >
            ...
            <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex flex-col">
                    <span className="text-[14px] font-medium text-black">Start Date</span>
                ...
                <div className="flex flex-col items-end gap-1">
                    <span className="text-[14px] font-medium text-black">End Date</span>
                    <span className="text-[14px] font-medium text-[#8B8B8B]">
                        {task.due_date
...
const PERIOD_OPTIONS = ["Period", "This Week", "This Month", "This Quarter", "Show All"];

export default function MytaskBM() {
    const [searchParams] = useSearchParams();
    const { pathname } = useLocation();
    const ismy = searchParams.get("condition") === "1" || pathname.endsWith("/my");
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            .then(([tasksRes1, tasksRes2, empRes, projRes1, projRes2]) => {
                const t1 = (tasksRes1.data.tasks ?? []).map(t => ({ ...t, source: "In House" as const }));
                const t2 = (tasksRes2.data.tasks ?? []).map(t => ({ ...t, source: "Outsource" as const }));
                setList([...t1, ...t2]);
                ...
                const p1 = (projRes1.data.projects ?? []).map(p => ({ ...p, source: "In House" as const }));
                const p2 = (projRes2.data.projects ?? []).map(p => ({ ...p, source: "Outsource" as const }));
                setProjects([...p1, ...p2]);
            })
            .catch((err) => {
    ...
    const getEmployeeOptions = () => {
        if (!selectedProject || ["Select Projects", "Show All", "Projects"].includes(selectedProject)) {
            return ["Select Employee", "Show All", ...employees.map((e) => e.full_name)];
        }
        const proj = projects.find((p) => p.project_name === selectedProject);
        if (!proj) return ["Select Employee", "Show All", ...employees.map((e) => e.full_name)];
    ...
    const counts = {
        ...
        in_progress: allTasks.filter((t) => normalizeStatus(t.status, t.Approval) === "in_progress").length,
        completed: allTasks.filter((t) => normalizeStatus(t.status, t.Approval) === "completed").length,
    };

    const tasksByStatus = {
        todo: allTasks.filter((t) => normalizeStatus(t.status, t.Approval) === "todo"),
        in_progress: allTasks.filter((t) => normalizeStatus(t.status, t.Approval) === "in_progress"),
        ...
        <div className="h-full min-h-0 flex flex-col overflow-hidden px-4 md:px-8 py-4 bg-white">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                    ...
                    <div ref={dropdownsContainerRef} className="flex flex-wrap items-center gap-2">
                        ...
                        <button
                            ...
                            className="inline-flex items-center gap-2 rounded-md bg-[#DD4342] px-4 py-2 text-[14px] font-medium text-white shadow-sm cursor-pointer hover:bg-[#c13a3a] transition-colors"
                        >
            ...
            <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar pr-2 -mr-2">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-8">
                    <div
                        className="space-y-4 min-h-[200px] p-1"
