...
def client_login():
    ...
    with conn.cursor() as cur:
        cur.execute("UPDATE clientinformation SET failed_login_attempts = 0, account_locked_until = NULL WHERE email = %s", (email,))
        
        # Track device
        ip_addr = request.remote_addr or ""
        device_info = request.user_agent.string or ""
        
        cur.execute("SELECT id FROM user_devices WHERE email = %s AND user_type = %s AND ip_address = %s AND device_info = %s", (email, "client", ip_addr, device_info))
        if not cur.fetchone():
            cur.execute("INSERT INTO user_devices (email, user_type, ip_address, device_info) VALUES (%s, %s, %s, %s)", (email, "client", ip_addr, device_info))
            try:
                from utils.mailer import send_new_device_alert
                send_new_device_alert(email, full_name, ip_addr, device_info, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
            except Exception:
            ...
            cur.execute("UPDATE user_devices SET last_login = %s WHERE email = %s AND user_type = %s AND ip_address = %s AND device_info = %s", (datetime.now(), email, "client", ip_addr, device_info))
        conn.commit()
