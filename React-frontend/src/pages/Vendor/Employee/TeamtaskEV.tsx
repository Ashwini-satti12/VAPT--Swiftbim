    ...
    useEffect(() => {
        ...
        if (selectedProj) {
            api.post<{ success: boolean; modules: { label: string }[] }>("/api/projects/filters/modules", { projectId: selectedProj.id })
                .then(({ data }: { data: { modules: { label: string }[] } }) => {
                    setModules(data.modules.map((m: { label: string }) => m.label));
                })
                .catch(() => setModules([]));
        }
                        ...
                        <form
                            ...
                            onSubmit={(e) => {
                                ...
                                if (isEditing && existing) {
                                    ...
                                    api.post<{ success: boolean; task_id?: number }>('/api/tasks', payload).then((res: { data: { success: boolean; task_id?: number } }) => {
                                        if (res.data.success && res.data.task_id) {
                                            handleFiles(res.data.task_id);
                                            api.get<{ tasks?: Task[] }>("/api/tasks", { params: { condition: isTeam ? "1" : "0" } })
                                                .then((r: { data: { tasks?: Task[] } }) => setList(r.data.tasks ?? []));
                                        }
                                    });
                                }
