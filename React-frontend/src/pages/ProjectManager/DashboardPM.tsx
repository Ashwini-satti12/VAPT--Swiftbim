      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 py-4 overflow-visible lg:overflow-hidden">
                    ...
                    <div key={id} className="mb-6">
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        {projectTasks.map((task) => {
                          ...
                          return (
                            <div key={task.id} className="flex items-center gap-3 sm:gap-5 p-3 sm:p-5 bg-[#F8F8F8] rounded-xl border border-slate-200/80 shadow-sm relative">
                              <div className="relative w-14 h-14 sm:w-20 sm:h-20 flex items-center justify-center shrink-0">
                                ...
                                <span className="absolute text-[8px] sm:text-[10px] font-bold text-black font-mono">{countdown}</span>
                              </div>
                              <div className="flex-1 min-w-0 pr-2">
                                <h3 className="text-base sm:text-xl font-bold text-black truncate mb-0.5">{task.task_name ?? 'Task'}</h3>
        ...
        <div className="lg:col-span-1 flex flex-col h-[500px] sm:h-[560px] lg:h-full overflow-hidden">
          <div className="bg-white rounded-md border border-[#AEACAC52] pl-4 pb-4 pr-0 shadow-sm flex flex-col h-full min-h-0">
            ...
            {/* Month & Year navigation — same as DashboardTD */}
                  ...
                  celebrations.map((event: CelebrationEvent, i: number) => (
                    <div key={i} className="bg-[#F8F9FA] p-4 sm:p-5 rounded-xl border border-transparent hover:border-slate-200 transition-all flex flex-col relative">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className={`text-[9px] sm:text-[10px] px-2 sm:px-3 py-1.5 rounded-md font-bold uppercase tracking-widest leading-none font-gantari ${event.type === 'birthday' ? 'bg-[#FFF3E0] text-[#E65100]' : event.type === 'work_anniversary' ? 'bg-[#E7F6EA] text-[#2D8A39]' : 'bg-[#E3F2FD] text-[#1565C0]'
                          }`}>
                      ...
                      {event.type === 'birthday' && (
                        <>
                          <p className="text-[13px] sm:text-[15px] font-semibold text-slate-700 mb-1 font-gantari">Happy Birthday 🎂</p>
                          <p className="text-xs sm:text-sm text-slate-400 leading-relaxed font-gantari">Wishing you a wonderful birthday! May this year bring you happiness and success.</p>
                        </>
                      )}
                      ...
                      {event.type === 'project_due' && (
                        <>
                          <p className="text-[13px] sm:text-[15px] font-semibold text-slate-700 mb-1 font-gantari">Project Due</p>
                          <p className="text-xs sm:text-sm text-slate-400 leading-relaxed font-gantari">Project "{event.project_name}" is due on {event.due_date}.</p>
                        </>
                      )}
                    </div>
                  ))
